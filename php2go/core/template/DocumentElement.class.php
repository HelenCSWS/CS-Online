...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/template/DocumentElement.class.php,v 1.16 2005/07/20 22:41:18 mpont Exp $
// $Date: 2005/07/20 22:41:18 $
...
class DocumentElement extends Template
{
	...
	//!-----------------------------------------------------------------
	function parse() {
		$saveIncludeData = $this->Parser->tplInclude;
		$this->Parser = new TemplateParser($this->contentBuffer, T_BYVAR);
		$this->Parser->tplInclude = $saveIncludeData;
		parent::parse();		
	}
	...
	function generateFromQuery($blockName, $sqlCode, $connectionId=NULL) {
		if (!parent::isBlockDefined($blockName)) {
			...
			if ($Rs =& $Db->query($sqlCode)) {
				...
				parent::setCurrentBlock(TP_ROOTBLOCK);
			}
