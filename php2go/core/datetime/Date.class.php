...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/datetime/Date.class.php,v 1.21 2005/08/30 14:02:56 mpont Exp $
// $Date: 2005/08/30 14:02:56 $
...
class Date extends PHP2Go
{
	...
	// @return 		int Diferença em dias para as duas datas ou -1 se
	// 				alguma das datas possuir formato incorreto ou
	// 				for inferior a 01/01/1970
	// @static
	//!-----------------------------------------------------------------
	function getDiff($dateM, $dateS) {
		// Calcula o número de dias da diferença
		return TypeUtils::parseIntegerPositive(Date::dateToDays($dateS) - Date::dateToDays($dateM));
	}
	...
	function fromEuroToSqlDate($date, $preserveTime=FALSE) {
		if (Date::isEuroDate($date, $regs)) {
			...
			if ($preserveTime && isset($regs[4]) && isset($regs[5]) && isset($regs[6]))
				$res .= " $regs[4]:$regs[5]:$regs[6]";
			return $res;
		} else {
			return $date;
		}
	...
	// @desc		Retorna o nome do mês atual
	// @access		public	
	// @access		public
	// @param		ts int	Timestamp utilizado como referência para cálculo do mês
	// @return		string Nome do mês para a data fornecida. Se ela não for passada como parâmetro,
	//				será buscado o mês atual
	// @static	
	//!-----------------------------------------------------------------
	function monthName($ts=0) {
		...
		if ($ts <= 0)
			...
			$date = TypeUtils::parseInteger($ts);
		$month = date('n', $date);
