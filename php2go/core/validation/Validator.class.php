...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/validation/Validator.class.php,v 1.14 2005/08/30 14:14:27 mpont Exp $
// $Date: 2005/08/30 14:14:27 $
	...
	function validateField(&$Field, $path, $params = NULL, $userMessage = NULL) {
		...
		if (TypeUtils::isInstanceOf($Field, 'formfield')) {
			...
			// validação de valor do campo
				...
				if ($result !== FALSE) {
					if (TypeUtils::isArray($value) && $value === $currentValue)
						$Field->setValue($value);
					if (TypeUtils::isString($value) && strcmp(strval($value), strval($currentValue)))
						$Field->setValue($value);
					$Field->setValue($value);
				}
				return $result;
