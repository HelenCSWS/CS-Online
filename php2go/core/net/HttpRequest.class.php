...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/net/HttpRequest.class.php,v 1.20 2005/07/20 22:46:32 mpont Exp $
// $Date: 2005/07/20 22:46:32 $
...
class HttpRequest extends PHP2Go
{
	...
	// @static		
	//!-----------------------------------------------------------------
	function getVar($variableName, $where = 'all', $searchOrder = '') {
		$req =& HttpRequest::getInstance();
		$searchOrder = (strspn($searchOrder, 'EGPCSOR')) ? $searchOrder : System::getIni('variables_order');
		$return = NULL;
	...
	//!-----------------------------------------------------------------
	function getHeaders() {
		$headersFunction = (function_exists('apache_request_headers')) ? 'apache_request_headers' : 'getallheaders';
		return $headersFunction();
	}
	...
	function serverHostName() {
		$port = Environment::has('HTTP_PORT') ? Environment::get('HTTP_PORT') : 80;
	...
	function scriptInfo() {
		...
		return array('path' => $scriptName, 'base' => $scriptBase, 'file' => $scriptFile);
	}
	...
	//!-----------------------------------------------------------------
	function url() {
		return HttpRequest::protocol() . '://' . HttpRequest::serverName() . HttpRequest::basePath();
	}
	...
	//!-----------------------------------------------------------------
	function uri() {
		$base = HttpRequest::protocol() . '://' . HttpRequest::serverName();
	...
	//!-----------------------------------------------------------------
	// @function	HttpRequest::checkEncoding
	// @desc 		Verifica se o servidor Web aceita um determinado
	// 				padrão de codificação/compressão
	// @param 		encoding string		Padrão de codificação
	// @return		bool
	// @deprecated	Este método está depreciado. Utilize a classe php2go.net.UserAgent
	// @see			UserAgent::matchAcceptList
	// @static
	//!-----------------------------------------------------------------
	function checkEncoding($encoding) {
		$Agent =& UserAgent::getInstance();
		return (bool)$Agent->matchAcceptList($encoding, 'encoding');
	}		

	//!-----------------------------------------------------------------
	// @function	HttpRequest::isBrowser
	// @desc 		Compara o browser atual com as definições passadas como parâmetro
	// @access		public
	// @param 		abrev string	Abreviação do nome do browser
	// @return		bool
	// @deprecated	Este método esta depreciado. Utilize a classe php2go.net.UserAgent	
	// @see			UserAgent::matchBrowser
	// @see			UserAgent::matchBrowserList
	// @static
	//!-----------------------------------------------------------------
	function isBrowser($abrev) {
		$Agent =& UserAgent::getInstance();
		return $Agent->matchBrowserList($abrev);
	}
	
	//!-----------------------------------------------------------------
	// @function	HttpRequest::getBrowserName
	// @desc		Busca a identificação do browser do cliente
	// @access		public
	// @return 		string Abreviação do nome do browser utilizado
	// @deprecated	Este método esta depreciado. Utilize a classe php2go.net.UserAgent
	// @see			UserAgent::getBrowserName
	// @see			UserAgent::getBrowserFullName
	// @static
	//!-----------------------------------------------------------------
	function getBrowserName() {
		$Agent =& UserAgent::getInstance();
		return $Agent->getBrowserName();
	}	
	
	//!-----------------------------------------------------------------
	// @function	HttpRequest::getBrowserInfo
	// @desc		Monta um vetor de informações sobre o browser cliente, contendo
	//				nome, versão, porções maior e menor da versão, ...
	// @access		public
	// @return		array Vetor contendo as informações sobre o browser
	// @deprecated	Este método esta depreciado. Utilize a classe php2go.net.UserAgent
	// @static
	//!-----------------------------------------------------------------
	function getBrowserInfo() {
		...
		return array(
			...
			'fullver' => $Agent->fullVersion,
			'major' => $Agent->majorVersion,
			'minor' => $Agent->minorVersion,
			'verlet' => $Agent->verlet,
			'gecko' => $Agent->browser['gecko'],
			'build' => $Agent->features['gecko']
		);
	}
	...
	function _fetchVar($variableName, $where) {
		...
		$arrayContent = TypeUtils::toArray($arrayContent);
		if (preg_match("/([^\[]+)\[([^\]]+)\]/", $variableName, $matches)) {
			if (isset($arrayContent[$matches[1]]) && TypeUtils::isArray($arrayContent[$matches[1]])) {
				return @$arrayContent[$matches[1]][$matches[2]];
			}
