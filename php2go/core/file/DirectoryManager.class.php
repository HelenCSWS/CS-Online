...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/file/DirectoryManager.class.php,v 1.11 2005/06/08 22:01:04 mpont Exp $
// $Date: 2005/06/08 22:01:04 $
...
class DirectoryManager extends FileSystem
{
	...
	function DirectoryManager() {
		parent::FileSystem();
		parent::registerDestructor($this, '_DirectoryManager');
	}
	
	//!-----------------------------------------------------------------
	// @function	DirectoryManager::_DirectoryManager
	// @desc		Destrutor da classe, libera os handles obtidos para
	//				a leitura de diretórios
	// @access		public
	// @return		void
	//!-----------------------------------------------------------------
	function _DirectoryManager() {
	...
	// @access		public
	// @return		array Retorna uma árvore de objetos do tipo DirectoryEntry, representando
	//				o contéudo do diretório atual, incluindo arquivos e subdiretórios
	// @note		O diretório utilizado para construir a árvore retorna na forma
	//				da raiz da árvore construída por este método
	//!-----------------------------------------------------------------
	function &getContentTree() {
		if (!isset($this->currentHandle) || !TypeUtils::isResource($this->currentHandle)) {
			return FALSE;
		} else {
			$root =& new DirectoryEntry($this->getParentPath(), $this->currentAttrs['lastDir']);
			$this->rewind();
			$this->_getChildren($root);
			return $root;
		}
