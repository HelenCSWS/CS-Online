...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/util/Properties.class.php,v 1.3 2005/09/01 15:48:10 mpont Exp $
// $Date: 2005/09/01 15:48:10 $
...
class Properties extends PHP2Go
{
	...
	function matchValue($key, $pattern, $section=NULL) {
		$tmp = $this->readKey($key, NULL, $section);
	...
	function getArray($key, $separator='|', $fallback=array(), $section=NULL) {
		$tmp = $this->readKey($key, $fallback, $section);
		if ($tmp != $fallback)
	...
	function setArray($key, $value, $glue='|', $section=NULL) {
		...
		$this->writeValue($key, $value, $section);
	}
