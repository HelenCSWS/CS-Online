...
//Array ( [1] => 8742 [2] => 30028400 [3] => CHRISTOPHER STE [4] => 5108 [5] => EXPATRIE RESERV [6] => 750 [7] => $22.36 [8] => $268.32 [9] => 8 [10] => 0 [11] => CS )  /*	  1. order id		2.licensee_no -- Required field;	3.Customer_name -- Required field 	  4. cspc code - Required field;	5. wine name - Required field 	  6. Bottle size 	7. display price 	  8 caseper price	9 allocation cases	  10. B/O		11. unit	  
	...
	//	$this->getVenderHtmlSalesData(126); //Bench 1775
	}
   ...
   //Array ( [1] => 31096 [2] => DEAD LETTER OFF [3] => 750 [4] => 12 [5] => 1 [6] => CS [7] => 71950800 [8] => KENSINGTON WINE MARKET [9] => $273.00 [10] => $22.75 )
	...
	function getVenderHtmlSalesData($estate_id=null)
    {
		...
		else if ($estate_id ==126) //bench 1775
			$fileName ="ab_sales_bench.html";
   ...
   function getBCWineryABSalesSummary($report_month="",$report_year="",$estate_id="")
   {
	...
	function getVenderSalesFromDB($report_month="",$report_year="",$estate_id="")
	{
        ...
        $pagedRS->setCurrentPage(1);
	 	...
	 	if($estate_id==150) // BC Estate
			...
			from $table_name st 
			left outer join customers c on st.licensee_no = c.licensee_number  and c.deleted =0
    ...
    function getInventoryHtmlData($estate_id)
    {
		...
		if($estate_id == 126)//enotecca
			$fileName ="ab_inventory_bench.html";	
		else
		{
		...
		if($nSize!=0)
		{
			...
			//CSPC1 DESCRIPTION2 SIZE3 UNITS4 $/UNIT5 OH_CS OH_UN HELD COM ALLOC10 AV_CS11 ON_ORD SOLD_TD 
 			...
 			//Array ( [1] => Array ( [1] => 319996 [CSPC] => BENCH1775 CHILL [DESCRIPTION] => 750 [SIZE] => 12 [UNITS] => $12.62 [$/UNIT] => 53 [OH_CS] => 0 [HELD] => 1 [COM] => 0 [ALLOC] => 52 [AV_CS] => 0 [ON_ORD] => 0 [SOLD_TD] => 912999 )
	...
	{	
		...
		$sql ="SELECT am.*, date_format(am.allo_date,'%m-%Y-%d') format_date
				from history_ab_monthly_allo am left outer join customers cm on am.license_no=cm.licensee_number and cm.deleted =0
	...
	{	
		...
		$sql ="SELECT am.*, date_format(am.allo_date,'%m-%Y-%d') format_date
				from ab_monthly_allo am left outer join customers cm on am.license_no=cm.licensee_number and cm.deleted=0
				left outer join users_customers uc on cm.customer_id=uc.customer_id   
	...
	/*
	  ...
	  */
		...
		{		 
		 	...
		 	if($estate_id ==175 or $estate_id ==96)
		 		$table_name = "ab_sales_bc";		 		
	    ...
	    /* echo test;
	     ...
	     $bccAddress ="";
	     $fromAddress = "Christopherstewart@christopherstewart.com";
	     ...
	     $from_name = "helen";
	      ...
	      F60Common::_sendEmail($toAddress,$bccAddress,$fromAddress,$emailSubject,$emailContent,$from_name);	//$toAddress ,$bccAddress,$fromAddress $emailSubject, $emailContent	  */
	       ...
	       $mail = new MailMessage(); 
	        ...
	        $mail->setFrom("Christopherstewart@christopherstewart.com"); 
    ...
    function emailReport()
    {
        ...
        $subject = "Alberta Inventory reports";
        $body = " ";

		$current_month = date(m);
		$current_year = date(Y);

		$sale_month = intval($current_month)-1;
		if($sale_month!=0)        
		{
        ...
        {
         	$sale_month = 12;
			$sale_year = intval($current_year)-1;
		}
       ...
       $fileName = $excelReport->getReportFile($sale_month,$sale_year);
	   ...
	   import('Form60.exportreports.excelBCInABVenderReport');
	  		 ...
	  		 /*  //removed by Helen,CC Jentsch is not available since May 01,2017
       $excelReport = new excelBCInABVenderReport(false,true);
       $fileName = $excelReport->getReportFile($sale_month,$sale_year,175);
       array_push($arrayAttachFileNames,$fileName);*/
       
     /*
	  // removed by Helen Enotecca is not available since June 2016
	   $excelReport = new excelBCInABVenderReport(false,true);
       ...
       array_push($arrayAttachFileNames,$fileName);*/
        ...
        /*  //removed by Helen, Bench 1775 is not available since May 01,2011
       $excelReport = new excelBCInABVenderReport(false,true);
       //$fileName = $excelReport->getReportFile($sale_month,$sale_year,118);
       $fileName = $excelReport->getReportFile($sale_month,$sale_year,126);
       array_push($arrayAttachFileNames,$fileName);
       */
