...
class excelABStorePenReport 
{
    ...
    function _writeData(&$workbook, & $sp, & $row, $infoData)
    {
		...
		while ($salesData=$infoData["sales_data"]->fetch())
		{
			...
			$values = array(
							...
							array("data"=>$salesData["product_name"],"format"=>$CalibriNormalBorderLeft ), 
							array("data"=>$salesData["store_number"], "format"=>$CalibriNormalBorderNumRight)																																									); 
    ...
    function _writeTitle(& $workbook, & $sp, & $row, $titleText)
    {
        ...
        $this->_writeCell($sp, array("data"=>$titleText), $row, "A", $CalibriBoldUnderlined); 
        $row++;
        $row++;
