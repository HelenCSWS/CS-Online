...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/session/SessionObject.class.php,v 1.17 2005/09/01 20:02:46 mpont Exp $
...
class SessionObject extends SessionManager 
{
	...
	function SessionObject($name) {
		...
		if (parent::isRegistered($name)) {
			if (TypeUtils::isArray($_SESSION[$name])) {
				$props = $_SESSION[$name];
	...
	function &getTimeCounter($tName, $throwError=TRUE) {
		if (!isset($this->timeCounters[$tName])) {
			...
			return $this->timeCounters[$tName];
		} 
	} 
