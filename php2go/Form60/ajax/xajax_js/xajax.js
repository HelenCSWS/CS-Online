...
function containsValue(ar,valueToCheck){for(var i=0;i<ar.length;i++){if(ar[i]==valueToCheck)return true;}
return false;}
...
else{if(containsValue(xajax.responseErrorsForAlert, r.status)){var errorString="Error: the server returned the following HTTP status: "+r.status;errorString+="\nReceived:\n"+r.responseText;alert(errorString);}
