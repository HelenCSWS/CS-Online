...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/file/FileManager.class.php,v 1.14 2005/06/08 22:01:29 mpont Exp $
// $Date: 2005/06/08 22:01:29 $
...
class FileManager extends FileSystem
{
	...
	function FileManager() {
		parent::FileSystem();
		parent::registerDestructor($this, '_FileManager');
	}

	//!-----------------------------------------------------------------
	// @function	FileManager::_FileManager
	...
	function getCurrentPointer() {
		return isset($this->currentFile) && TypeUtils::isResource($this->currentFile) ? $this->currentFile : FALSE;
	}
	...
	function open($filePath, $mode = FILE_MANAGER_READ_BINARY, $lockFile = FALSE) {
		...
		if (!isset($pointers[$filePath][$mode]) || !TypeUtils::isResource($pointers[$filePath][$mode])) {
			...
			if (TypeUtils::isFalse($pointers[$filePath][$mode])) {
				if ($this->throwErrors) 
	...
	function getCurrentPosition() {
		if (!TypeUtils::isResource($this->currentFile)) {
			return FALSE;
		} else {
			$fp =& $this->currentFile;
			return ftell($fp);
	...
	function getAttribute($attributeName) {
		if (!isset($this->currentFile) || !TypeUtils::isResource($this->currentFile))
			return FALSE;
		else
	...
	function read($size = FILE_MANAGER_DEFAULT_BLOCK, $lockFile = FALSE) {
		if (!TypeUtils::isResource($this->currentFile)) {
			...
			return feof($fp) ? FALSE : fread($fp, max(TypeUtils::parseInteger($size), 1));
		}
	}
	...
	function write($string, $size = 0, $lockFile = FALSE) {
		if (!is_scalar($string) || !TypeUtils::isResource($this->currentFile)) {
			return FALSE;
		} else {
			$fp =& $this->currentFile;
			if ($lockFile == LOCK_SH || $lockFile == LOCK_EX) {
		...
		foreach ($files as $file) {
			if (FileSystem::exists($file)) {
				if (function_exists('file_get_contents')) {
					...
					$this->open($file, FILE_MANAGER_READ_BINARY);
	...
	function truncate($size, $lockFile = FALSE) {
		if (!TypeUtils::isResource($this->currentFile)) {
			...
			$fp =& $this->currentFile;
			$truncSize = TypeUtils::parseIntegerPositive($size);
			if ($lockFile == LOCK_SH || $lockFile == LOCK_EX) {
				$this->lock($fp, $lockFile);
			}			
	...
	function touch($time=NULL) {
		if (isset($this->currentPath)) {			
			$time = TypeUtils::ifNull($time, time());
		...
		if (!empty($pointers)) {			
			foreach($pointers as $path => $value) {
				foreach($value as $mode => $pointer) {
					if (TypeUtils::isResource($pointer) && get_resource_type($pointer) != 'Unknown')
						@fclose($pointer);					
				}
			}			
		}				
