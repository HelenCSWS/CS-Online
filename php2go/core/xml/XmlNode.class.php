...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/xml/XmlNode.class.php,v 1.11 2005/08/30 14:15:33 mpont Exp $
// $Date: 2005/08/30 14:15:33 $
...
class XmlNode extends AbstractNode
{
	...
	// @param		name string				Nome procurado
	// @param		&elementArray array		Vetor por onde retornam os resultados encontrados
	// @access		public
	// @return		void
	//!-----------------------------------------------------------------
	function getNamedItem($name, &$elementArray) {
		if ($this->name == $name)
			$elementArray[] =& $this;
		if ($this->hasChildren())
			for ($i=0; $i<$this->childrenCount; $i++)
				$this->children[$i]->getNamedItem($name, $elementArray);
	}	
	...
	function render($lineEnd='', $depth=0, $indent='') {
		...
		if ($this->hasChildren() || $this->hasData()) {
			...
			if ($this->hasData()) {
				if (strlen($this->value) != strlen(htmlspecialchars($this->value))) {
					$cdata = TRUE;
