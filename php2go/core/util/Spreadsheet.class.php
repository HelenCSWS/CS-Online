...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/util/Spreadsheet.class.php,v 1.13 2005/05/27 14:00:30 mpont Exp $
// $Date: 2005/05/27 14:00:30 $
...
class Spreadsheet
{
	...
	function download($fileName) {
		...
		if (!HttpResponse::headersSent()) {
			...
			HttpResponse::download($fileName, $this->streamSize);
...
function debugTxt($text)
	...
	function _transformDate($date) {
		if (!System::loadExtension('calendar')) {
			...
			} else if (Date::isUsDate($date, $dateParts) || Date::isSqlDate($date, $dateParts)) {
				$dtVal = juliantojd($dateParts[2], $dateParts[3], $dateParts[1]) - SPRSH_DATE  + 1;
			}
			$dtVal += ( TypeUtils::parseInteger($dateParts[5]) / 24 )
					+ ( TypeUtils::parseInteger($dateParts[6]) / 1440 )
					+ ( TypeUtils::parseInteger($dateParts[7]) / 86400 );
			return $dtVal;
		}
	}
