...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/xml/XmlDocument.class.php,v 1.10 2005/08/30 14:15:01 mpont Exp $
// $Date: 2005/08/30 14:15:01 $
...
class XmlDocument extends PHP2Go
{
	...
	//!-----------------------------------------------------------------
	// @function	XmlDocument::&getElementById
	// @desc		Busca o elemento identificado por um determinado ID
	// @access		public
	// @return		XmlNode object Retorna o objeto buscado ou NULL se não encontrar
	//!-----------------------------------------------------------------
	...
	//!-----------------------------------------------------------------
	// @function	XmlDocument::&getElementsByTagName
	// @desc		Monta um vetor contendo elementos que possuem um determinado nome (nome da tag)
	// @access		public
	// @param		tagName string		Nome buscado
	// @return		array
	//!-----------------------------------------------------------------
	function &getElementsByTagName($tagName) {
		$result = array();
		if (!TypeUtils::isNull($this->DocumentElement))
			$this->DocumentElement->getNamedItem($tagName, $result);
		return $result;
	}
	...
	function render($lineEnd='', $indent='') {
		...
		if (!TypeUtils::isNull($this->DocumentElement)) {
			...
			// folhas de estilo
			for ($i=0; $i<sizeOf($this->styleSheet); $i++)
