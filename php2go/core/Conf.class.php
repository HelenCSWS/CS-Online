...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/Conf.class.php,v 1.12 2005/07/25 13:24:16 mpont Exp $
// $Date: 2005/07/25 13:24:16 $
...
class Conf
{
	...
	function loadConfig($configModule) {
		$this->config = importFile($configModule, TRUE);
	}
	...
	function getConfig($configName) {
		return findArrayPath($this->config, $configName, '.', FALSE);
	}
	...
	function getConnectionParameters($connectionId=NULL) {
		...
		if (TypeUtils::isArray($connections)) {
			...
			if (!TypeUtils::isNull($connectionId, TRUE)) {
				if (isset($connections[$connectionId]))
					$params = TypeUtils::toArray($connections[$connectionId]);
			} else {
			...
			$params['PERSISTENT'] = TypeUtils::toBoolean(@$params['PERSISTENT']);
		...
		if (empty($params['USER']) || empty($params['BASE']) || empty($params['TYPE']))
