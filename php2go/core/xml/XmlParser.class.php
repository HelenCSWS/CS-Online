...
class XmlParser extends PHP2Go
{
	...
	// @desc		Método estático para a criação de um parser XML utilizando
	//				um determinado tipo de codificação para o código XML
	// @access		public
	// @param		srcEncoding string 	"NULL" Codificação a ser utilizada pelo parser
	// @param		&xmlSource string	Código XML
	// @param		optionFlags array	Conjunto de opções para o parser
	...
	function createParser($srcEncoding=NULL, &$xmlSource, $optionFlags=array()) {
		...
		if (!TypeUtils::isNull($srcEncoding)) {
			// codificação definida explicitamente
			$parser = xml_parser_create($srcEncoding);		
		} elseif (System::isPHP5()) {
			// php5 - possui auto detecção de source encoding
			...
			if (preg_match('/<?xml.*encoding=[\'"](.*?)[\'"].*?>/m', $xmlSource, $matches))
				...
				$srcEncoding = 'UTF-8';
			// se for uma das codificações válidas para a xml entension
