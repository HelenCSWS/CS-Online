...
//---------commission levles -------------------
function loadCommlevels(type_id)
{
...
/*	var totalTypeNumber=4;
	var i=1;
	
	var tdName="";
	for(i=1;i<=totalTypeNumber; i++)
	{
		tdName = "tdSectionCommission_type_"+i;
		$(tdName).style.display="none";
	}
	
	tdName ="tdSectionCommission_type_"+type_id;
...
{
 	var commissionTypeId=$("lkup_sales_commission_type_id").value;
 	
 	var totalLevels=10;
 	
 	if(commissionTypeId==3)
 		totalLevels=5
 // alert(id);
    var nameCheck="bcldb_chklevel"+id;
...
function loadCommlevelsType2(type_id)
{
	var totalLevels =10;
	...
	if(totalLevels-levels>0)
	{
		for (id=(totalLevels-((totalLevels-levels-1)));id<=totalLevels;id++)
		   {
		        bcldb_checkLevels(id,true);
		   }
	...
	if(type_id==3)
	{
		var radio_id = $("lkup_commission_sales_sum_type_id").value-1;
		
	
		
		salesCommission.rdoSumType[radio_id].checked=true;;
	}
...
function setCases(ctlCases,ctId)
{
    ...
    if(ctlCases.value==""||ctlCases.value==null)
    {
      if(ctId!=1)
      {
        if ( document.getElementById(chkLevel).checked )
	 ...
	 /*
   	id: 0: go to select user commission type page: with same user
   	    1: go to select user commission type page: with default user
   */
 
    function openPage(isNextUser)
   	{
		...
		var link = "main.php?page_name=selectCommissionType&user_id="+user_id+"&lkup_sales_commission_type_id="+type_id+"&province_id="+province_id;
	...
	function setSumType(id)
	{
		$("lkup_commission_sales_sum_type_id").value=id;
	}
