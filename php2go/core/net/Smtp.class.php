...
// +----------------------------------------------------------------------+
//
// $Header: /www/cvsroot/php2go/core/net/Smtp.class.php,v 1.11 2005/07/20 22:29:21 mpont Exp $
// $Date: 2005/07/20 22:29:21 $
...
class Smtp extends SocketClient
{
	...
	function Smtp(){
		...
		parent::registerDestructor($this, '_Smtp');
	}
	...
	function _readResponse(&$responseCode, &$responseMessage) {
		if ($data = $this->_readData()) {
			$responseCode = StringUtils::left($data, 3);
			$responseMessage = StringUtils::right($data, strlen($data)-4);
			return TRUE;
		} else {
			return FALSE;
		}
