...
class F60ReportsBase extends Report
{
    var $contents;
    var $pageLink;
    var $idName;
    ...
    function F60ReportsBase($xmlFile, $templateFile, &$Document)
    {
       Report::Report($xmlFile, $templateFile, &$Document);
       Report::setPageSize(35);
    }
    ...
    function setRequestString($rqstr)
    {
...
array("col_wid", "loop_cell")), E_USER_ERROR, __FILE__, __LINE__);
						...
						//$this->sLink =$_SERVER["REQUEST_URI"] . '?customer_id=' .$data['customer_id'] . '">'
							if($key=="isRecieved")
							//<A href="javascript:viewDetails('{order_id}')">{order_date}</A>
								 //$sLink ="javascript:viewDetails('{order_id}')";
								//$sLink =="javascript:viewDetails()" . '">'; 
							{   
						
								if($data[$key]!="Delivered")
								{
									$order_id = $data["order_id"];
									$sLink="javascript:updateInvocieStatus($order_id)". '">';	
								}
								else
...
$this->Template->isVariableDefined('loop_cell.alt_style'))
	...
	function _generatePageLink($page, $order = '') 
	{
		if (isset($this->_order) && $order == $this->_order)
			...
			if ($this->is_print)
				$path =HttpRequest::basePath()."?page_name=F60printreport".$this->rqstr;
			else
			{
	        	
				$path =HttpRequest::basePath()."?page_name=F60Reports".$this->rqstr;
